...
Schema, entity, and member identifiers map deterministically to PascalCase. If distinct EXPRESS names would collide in the generated C# namespace or member surface, `STEP21EXP004` is reported at the source name and every output for that schema is withheld. Explicit redeclarations support direct entity subtype domains, closed entity-valued SELECT narrowing, exact `NUMBER` to `INTEGER`/`REAL`, same-kind entity aggregate narrowing with identical metadata, and `OPTIONAL` to required tightening. Entity and aggregate projections preserve object identity; SELECT projections preserve the selected leaf; numeric projections preserve the exact alternative without binary floating point. ISO-invalid redeclarations report source-located `STEP21EXP002` with `EXPRESS-BIND-INVALID-REDECLARATION`; legal-but-unimplemented or incomparable mappings report `STEP21EXP005`. Either result withholds the schema atomically. Invalid generation state propagates through importing schemas; independent valid schemas remain eligible. Cross-schema inheritance and entity properties use the target schema's fully qualified generated namespace.
