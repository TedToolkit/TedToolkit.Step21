...
- `SCHEMA_POPULATION` external-file triples retain location, optional last-visited timestamp, optional canonical Base64 digest, resolution status, and the resolved structure.
...
- A supplied digest requires a signature on the declaring structure and uses the first signature section's digest algorithm over the referenced Part 21 alphabet characters. Mismatch and unsupported algorithms fail atomically.
