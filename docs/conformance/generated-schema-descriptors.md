...
Physical `INTEGER` parameters do not satisfy an EXPRESS target that resolves to `REAL` during default strict binding.
`ExchangeStructureReadOptions.WithCompatibility(Part21ReadCompatibility.IntegerForReal)` is a deliberate opt-in
interoperability extension: generated hydration promotes those integers exactly to `RealValue` for direct attributes,
defined types, aggregate elements, and SELECT alternatives throughout the same resource graph. It does not change the
physical `ParameterValueKind`, the existing `NUMBER` alternatives, any other target-domain mapping, or the grammar.
Writing projects the typed `RealValue` through the normal canonical REAL representation. See
[ADR-0015](../adr/ADR-0015-opt-in-numeric-representation-compatibility.md) for the compatibility boundary.
