...
segment, escape, formatted numeric value, entity record, and CMS Base64 value reserves its remaining quota before it
is retained; a limit failure therefore performs work and allocation proportional to the configured bound rather than
materializing an unbounded intermediate value. CMS Base64 preflight uses both the per-envelope limit and the remaining
aggregate byte budget before decoding.
