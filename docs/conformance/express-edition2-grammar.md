...
| Expressions, operators, aggregate initializers, intervals, queries, qualifiers, calls, entity constructors, and literals | `expression` through `unaryOp`; non-empty `actualParameterList`; optional-expression `emptyEntityConstructorList` | `complete-expressions.exp`, `edition2-and-lexical.exp`, focused empty-constructor syntax/binding/generation tests |
...
EXPRESS uses the same physical identifier forms for several semantically distinct references. For example, `name(...)` can be a function call or entity constructor, and a bare or qualified name can denote an attribute, constant, parameter, variable, population, or enumeration. Choosing among them requires schema name binding and is not a context-free syntax decision. The `actual_parameter_list` production remains non-empty, while the separately governed entity-constructor production permits an optional expression list and therefore requires `EntityName()` for an entity with no explicit attributes. The grammar retains that empty form separately so binding accepts it only for an entity name and continues to reject an empty function application.

The syntax IR therefore records these forms honestly as `namedApplication` and `namedReference`, retaining every token and half-open, 1-based source span. Closed-set binding resolves their EXPRESS name class. The grammar does not use semantic actions, reflection, or execution, and failed parsing publishes diagnostics without a partial IR.
