...
- Applicable product intent: [`docs/product/README.md`](../product/README.md), approved in the working tree on 2026-08-21; commit pin pending
- Governing principles: [`docs/principles/architecture.md`](../principles/architecture.md) and [`docs/principles/engineering.md`](../principles/engineering.md), active in the working tree; commit pins pending
- Related ADRs: [`ADR-0001`](../adr/ADR-0001-schema-bound-entity-model.md), [`ADR-0002`](../adr/ADR-0002-roslynhelper-source-composition.md)
- Last approved revision: not yet approved
...
| AD-04 | How generated declarations represent EXPRESS entities and inheritance | Generate one interface and one record for every EXPRESS entity; express all schema inheritance through interfaces; generated records inherit only the runtime `Entity` base record | The maintainer confirmed this uniform projection on 2026-08-21 | Generated model shape | Resolved |
...
EXPRESS AdditionalFile (.exp)
  ...
  -> generated schema contracts, records, value types, and metadata
  -> depends only on TedToolkit.Step21 runtime

ISO 10303-21 input
  -> schema-neutral exchange-structure parser
  -> raw immutable exchange structure
  -> generated schema metadata binder
  -> ExchangeStructure containing generated records and direct entity references
  -> schema-aware writer
  -> ISO 10303-21 output
```
...
The runtime owns immutable representations for the standard concepts, including:
...
The raw writer emits a deterministic, syntactically conforming representation of the raw model for supported constructs. It does not promise byte-for-byte preservation of comments, whitespace, spelling, or original entity ordering.
...
- allocation and hydration factories that bind raw parameters to generated records in two phases;
- writers that project generated records back to standard parameters without reflection-based property discovery.

### Generated type system

Interfaces are the complete and exclusive representation of EXPRESS entity inheritance. Every EXPRESS entity produces both an interface and a record implementation.

- Each generated entity interface extends exactly its EXPRESS supertype interfaces.
- Each generated entity record inherits directly from the runtime `abstract record Entity` and implements its own generated interface. It does not inherit another generated entity record. Because the own interface already inherits all schema supertypes, repeating the transitive interface list on the record is unnecessary.
...
- Records for abstract EXPRESS entities are abstract; all instantiable generated entity records can be `sealed` because schema subtype polymorphism is carried by interfaces rather than record inheritance.
- Inherited schema-defined storage is generated exactly once in each concrete record projection. No arbitrary "primary supertype" exists.
- Explicit attributes are publicly get-only record properties. Entity-valued backing members permit generated non-public assignment during hydration and are immutable to ordinary consumers after publication. Derived and inverse attributes are not serialized properties; optional computed/navigation APIs may be generated separately only when their semantics are supported.
...
- Aggregates use immutable runtime record/value wrappers that retain LIST/SET/BAG/ARRAY kind, bounds, order, and uniqueness semantics.
...
1. Allocate every generated entity record and register the raw `EntityInstanceName -> object` mapping without requiring entity-valued properties to be complete.
...
Two-phase assignment requires generated private hydration machinery or backing fields. Public getters remain strongly typed. The parser does not publish an entity or typed model until all mandatory relationships are bound and validation succeeds. Public construction requires every mandatory property immediately; no public constructor, builder, setter, or result exposes a temporarily incomplete entity.

### Model ownership, construction, and mutation

`ExchangeStructure` is the non-generic C# representation of the standard exchange structure. It owns section context, maps local `EntityInstanceName` values to generated entity objects, and maintains reverse maps using CLR reference identity rather than record value equality. Schema identity is represented by the standard header/data-section concepts and generated descriptors rather than a single generic type argument; this does not exclude standard exchange structures involving multiple schemas. Records are complete whenever ordinary consumers can observe them; parser-only hydration state never crosses the parse-result boundary.

- Parsed records do not need to expose their entity instance names; the model retains that association.
- New entities may be registered with an explicit valid name or through a model allocator after construction.
- Replacement uses record `with` expressions followed by a checked model replace operation that updates identity and validates direct references.
- Removal fails while retained references would become invalid unless an explicit unchecked/raw workflow is selected.
...
- the Part 21 base visitor transforms the parse tree into the immutable schema-neutral `ExchangeStructure` representation;
...
- Require all mandatory properties for public construction; do not expose public incomplete-state APIs.
- Treat any generated .NET mapping attribute as non-normative library metadata; the first contract does not require one.
- Express all EXPRESS entity inheritance through generated interfaces; generated entity records inherit only `Entity` and implement their own interface.
- Every instantiable generated entity record is sealed; generated immutable value wrappers are record structs where appropriate.
...
- ADR-0001 proposes the interface/record/direct-reference/two-phase-hydration representation.
...
- A valid EXPRESS inheritance or mapping construct cannot be represented by the interface/record metadata model.
