...
<!-- approval-source: user-explicit-delete-switch-and-local-generation-only-2026-09-10 -->

## Goal and rationale

<!-- section: goal-rationale -->

Make the maintained AP203, AP214, and AP242 packages consume the strongest available authoritative EXPRESS publication directly from a verified, Git-ignored local cache, with no STEPcode-owned schema or licensing material and no local edits to official bytes. The maintainer explicitly accepts the resulting breaking AP203 contract and restricts EXP files to non-distributed code-generation inputs.

The governing decision is [ADR-0013](../../adr/ADR-0013-authoritative-express-schema-sources.md).
...
The active schemas currently come from a pinned STEPcode commit. AP203 and AP242 include local corrections, and all three schema directories retain STEPcode license and attribution files. After migration, active package inputs are hash-verified pristine downloads in an ignored local cache, compiler fixes own any required compatibility, and the tracked repository contains only the fetch manifest, provenance, and build guard—not the EXP bytes.
...
- AP242 must compile without the two local edits; fixes must be general EXPRESS behavior rather than filename- or schema-specific exceptions.
- The pristine AP242 edition 7 file currently produces two compiler type conflicts also reproduced from ISO SMRL v12; this is a compilation limitation, not a provenance mismatch. See [`ap242-compilation-status.md`](ap242-compilation-status.md). By maintainer decision, no corrected copy or overlay is permitted; the affected build remains blocked pending an authoritative correction or faithful general compiler support.
...
### AC-02 — Pristine publications compile without schema patches

<!-- acceptance-case: AC-02 -->

```gherkin
Scenario: Generate maintained schema packages
  Given the byte-exact authoritative EXP files
  When the repository compiler and source generator process all three schemas
  Then generation and C# compilation complete without schema-specific source edits or error diagnostics, while representative invalid equivalents of the newly supported constructs retain deterministic diagnostics
```
...
Scenario: Exercise maintained AP214 and AP242 fixtures
  ...
  Then their existing schema-appropriate observable results remain valid against the pristine source contracts
```
...
| AC-02 | Primary | All three pristine schemas bind, generate, and compile without error diagnostics; near-miss invalid forms of each new compiler accommodation retain their expected diagnostics | Run focused AP203, AP214, and AP242 compiler/generator tests, including positive official-expression and neighboring-invalid regression cases, then build the three package projects |
