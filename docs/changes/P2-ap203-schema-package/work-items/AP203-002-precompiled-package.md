...
| Evidence | Result |
| --- | --- |
| Candidate boundary | Work started from integration revision `94ee6e834f454ab6f2b957d0dc24f50fd9a453b1`. The final candidate SHA is reported in the worker handoff because a commit cannot contain its own identity. The candidate adds only the optional package project, solution and central-version wiring, package-consumer proof, package documentation, and this evidence. |
...
| AC-01 package-only proof | `dotnet run --project tests/TedToolkit.Step21.IntegrationTests --configuration Release -- --treenode-filter "/*/*/*/Should_build_and_run_the_precompiled_ap203_package_only_consumer"` passed 1/1. The temporary consumer references only `TedToolkit.Step21.Ap203`, supplies no `.exp` or `AdditionalFiles`, compiles and executes against `SchemaDescriptor`, `Product`, and `IBoundedPcurve`, and produces no generated `.g.cs` files. |
