...
independent `Ready` reviews; the reviewed AP203-001 range ends at `28ba730`. AP203-002 is integrated
with complete proof and a `Ready` review through `da17c4e`. Candidate revision `f66c33d` still contains
the start of AP203-004, which awaits reconciliation onto this delivery baseline.
