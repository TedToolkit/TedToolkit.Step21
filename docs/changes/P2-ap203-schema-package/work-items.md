...
Status evidence as of this revision: candidate integration revision `f66c33d` contains the AP203-001
and AP203-002 completion evidence, AP203-003 implementation commits `c456b01`/`4bf5510`, and the
start of AP203-004. `Implemented` means completion evidence is present in that candidate; `Implementing`
means implementation exists or has started but the authoritative completion record/review is not yet closed.
All candidate evidence still awaits integration into the current main branch.
...
| AP203-003 | Match standard schema identifiers without changing retained header text | Owns AC-03 / Owns AC-04 / Owns AC-08 / Supports AC-05 | None | In-memory identifier compatibility contract tests | Implementing | `work-items/AP203-003-schema-identifier-matching.md` |
