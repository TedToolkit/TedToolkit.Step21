...
The Part 21 parser's `exchangeFile` entry rule recognizes the complete ISO 10303-21:2016 Edition 3 clear-text section and token syntax, enforces the normative section order, and consumes EOF. `ExchangeStructure.Read` now atomically binds and validates the documented one-schema, one-simple-data-section slice through supplied generated descriptors, including forward/shared/cyclic local entity references; external resource acquisition, multiple/named data sections, complex instances, and operational anchor/signature behavior remain staged capabilities. See the [production-to-clause traceability](docs/conformance/part21-edition3-grammar.md), [atomic simple-read boundary](docs/conformance/atomic-simple-read.md), and [reference-hydration boundary](docs/conformance/reference-hydration.md).
