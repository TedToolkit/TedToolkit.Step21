// -----------------------------------------------------------------------
// <copyright file="Ap242FixtureProgram.cs" company="TedToolkit">
...
internal static class Ap242FixtureProgram
{
    private const string EditedProductName = "TedToolkit AP242 OCCT box 10x20x30 mm - edited";
    ...
    private static int Main(string[] arguments)
    {
        ...
        try
        {
            ...
            structure = ExchangeStructure.Read(source, [Ap242SchemaDescriptor.Instance]);
        }
        ...
        if (entities.Length != 170
            || product.Name.Value != "TedToolkit AP242 OCCT box 10x20x30 mm 1"
            || faces.Length != 6
        ...
        Console.WriteLine(
            $"AP242_FIXTURE_OK entities={entities.Length} products=1 faces={faces.Length} "
        ...
        {
            reread = ExchangeStructure.Read(source, [Ap242SchemaDescriptor.Instance]);
        }
        ...
        Console.WriteLine(
            "AP242_ROUND_TRIP_OK edit=product.name entities=170 faces=6 edges=12 "
        ...
        var expectedProductFailure = invalidValidation.Failures.Any(failure =>
            failure.Code == "AP242_MANAGED_MODEL_BASED_3D_ENGINEERING_MIM_LF.PRODUCT.FRAME_OF_REFERENCE.AGGREGATE_0.LOWER_BOUND"
        ...
        var expectedFaceFailure = invalidValidation.Failures.Any(failure =>
            failure.Code == "AP242_MANAGED_MODEL_BASED_3D_ENGINEERING_MIM_LF.FACE.BOUNDS.AGGREGATE_0.LOWER_BOUND"
        ...
        Console.WriteLine(
            $"AP242_INVALID_EDIT_REJECTED failures={invalidValidation.Failures.Count} output-bytes=0");

        ExchangeStructure? unsupported = null;
        try
        {
            using var source = File.OpenText(arguments[1]);
            unsupported = ExchangeStructure.Read(source, [Ap242SchemaDescriptor.Instance]);
            return 30;
        }
        catch (ExchangeStructureBindingException exception)
        {
            ...
            Console.WriteLine(
                $"AP242_EXTENSION_REJECTED code={diagnostic.Code} "
