...
internal sealed class PublicApiTests
{
    ...
    public async Task Should_match_approved_surface_when_validation_contract_is_inspected()
    {
        ...
        var approvedSnapshots = new[]
        {
            ...
            await File.ReadAllTextAsync(Path.Combine(approvedDirectory, "ISO21-001.approved.txt")),
            await File.ReadAllTextAsync(Path.Combine(approvedDirectory, "ISO21-002.approved.txt")),
            ...
            await File.ReadAllTextAsync(Path.Combine(approvedDirectory, "ISO21-009.approved.txt")),
            await File.ReadAllTextAsync(Path.Combine(approvedDirectory, "REAL-compatibility.approved.txt")),
        };
