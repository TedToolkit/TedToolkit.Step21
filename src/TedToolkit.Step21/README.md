...
- Signature sections are decoded as detached CMS. Optional per-read verification uses only explicit time, signer-certificate revocation input, custom roots, and acceptance policy; signed writing validates callback output against an independent canonical content snapshot and remains zero-output atomic on failure.
